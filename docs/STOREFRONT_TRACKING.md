...
4. The commission appears on David's dashboard, and the admin's **Recent attributed
   orders** row shows `attributed → DAVID`.

## Prerequisites

1. **Webhooks registered.** Shopify must be sending `orders/create` (and ideally
   `orders/paid`) to `https://<app>/api/webhooks/shopify`. If your store shows zero
   webhooks, nothing attributes — coupon or link.
2. **Shopify connected** in **Settings → Integrations** with the Admin API token
   **and** the API secret key (the secret verifies webhook signatures).
...
|---|---|
| `attributed → DAVID15` | ✅ commission created |
...
| `skipped — returning customer …` | program is new-customers-only |
